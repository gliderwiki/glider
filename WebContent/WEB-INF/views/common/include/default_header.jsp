			...
			$($('a[name="title_menu"]').eq(i)).mouseenter(function() {
				...
				if(title == '위키생성하기') {
					...
					$("#"+menuId).on("click", function(e) {
						...
						$.get("/api/authority/wiki", { spaceIdx: weSpaceIdx } , function(data){
							if(data == "SUCCESS") {
								location.href = thisUrl+weSpaceIdx;
							}else{
								GliderWiki.alert('위키생성','회원님은 위키를 생성할 수 없습니다.');
								return;
							}
						});
					});
				...
				if(title == '위키마크업활용') {
					...
					//	GliderWiki.alert('안내','위키 마크업 활용 페이지는 현재 준비중입니다.');
					//});
					$(this).bind("click", function() {
						$.markup_layer();
						return;
