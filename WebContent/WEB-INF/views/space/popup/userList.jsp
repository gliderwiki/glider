...
<html>
<head>
<link rel="stylesheet" href="http://gliderwiki.org/resource/glider/front/css/common.css">
			...
			<tr <c:if test="${(stat.count%2) == 0}">class="even"</c:if>>
				<td>
					...
					<c:when test="${authorityType == 'view'}">
						<input type="checkbox" name="userIdx" value="${list.we_user_idx}" data-user-name="${list.we_user_name}" <c:if test="${list.we_view_permit == 1}"> checked </c:if>>
					</c:when>
					<c:when test="${authorityType == 'edit'}">
						<input type="checkbox" name="userIdx" value="${list.we_user_idx}" data-user-name="${list.we_user_name}" <c:if test="${list.we_insert_permit == 1}"> checked </c:if>>
					</c:when>
					<c:otherwise>
						<input type="checkbox" name="userIdx" value="${list.we_user_idx}" data-user-name="${list.we_user_name}">
					</c:otherwise>
					</c:choose>
					${list.we_group_name}
				</td>
...
$(document).ready(function() {
	...
	$('.checkComplete').unbind('click').click(function(e){
		...
		var checkedViewUserCnt = $("input[name=userIdx]:checkbox:checked").length;

		if(checkedViewUserCnt == 0) {
			alert("하나라도 선택을 해야합니다.\n 선택을 하지 않으시려면 닫기버튼을 눌러주세요");
			return false;
		}

		$("input:checkbox[name^=userIdx]:checked").each(function(index){
			resultData += $(this).val();
			resultName += $(this).data("userName");

			if(index != (checkedViewUserCnt-1)) {
				resultData += ',';
				resultName += ',';
			}
		});
